...
toggle.addEventListener('click', () => {
  ...
  Array.from(lightText).map((lightEL1) => {
    lightEL1.classList.toggle('light');
  });
  Array.from(borders).map((bordEL) => {
    bordEL.classList.toggle('border-color');
  });
});
